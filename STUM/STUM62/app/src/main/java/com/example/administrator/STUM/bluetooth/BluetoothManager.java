...
*/
        ...
        query.whereGreaterThanOrEqualTo("watervolume",0);//물의 부피가 0보다 작은애는 에러다. 그래서 빼고 계산한다.
        query.addDescendingOrder("createdAt"); //만들어진순서로정렬후
        //query.whereEqualTo("month", 6);
        // query.whereEqualTo("day", 2);
        //query.setLimit(100); //갯수를 제한하고
                        ...
                        if (flag.equals("D")) { //D를 찾으면
                            if (i == 0 ) {
                                while (drinkList.get(i + 1).getString("drinkflag").equals("D")) {
                                    i++; //첫번째가 D면 이전 N을 찾을수 없으므로 그다음 D를 다 무시하고 그다음 N에 대하여 진행하도록한다...
                                }
                            }else {
                                ...
                                Log.d("D 이전 찾은 애들", year1 + "년 " + month1 + "월 " + day1 + "일 " + hour1 + "시 " + min1 + "분 "+ sec1+"초" + temp1 + "도 " + drunk1 + "ml");
                                ...
                                Log.d("D이후 찾은 애들", year2 + "년 " + month2 + "월 " + day2 + "일 " + hour2 + "시 " + min2 + "분 " + sec2+"초"+temp2 + "도 " + drunk2 + "ml");
                                ...
                                drunk_volume = drunk1 - drunk2;//1이 이전 2가 최신

                                ParseObject testDrunk = new ParseObject("testDrunk2");
